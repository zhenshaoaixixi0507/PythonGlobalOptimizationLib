...
fid = fopen('C:\Users\jingz732\source\repos\PythonGlobalOptimizationLib\PythonGlobalOptimizationLib\PythonGlobalOptimizationLib\DataDownload\return.csv');
...
function[sumLL]=Loglik(para,residual)
 ...
 for i=1:size(LL,1)
     ...
     LL(i,:)=0.5*log(2*pi)+0.5*log(sigma(i+1,:))+0.5*log(residual(i+1)^2/(sigma(i+1)));
 end
 sumLL=sum(LL);
